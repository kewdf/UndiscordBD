 ...
 * @author qdf
 * @version 0.0.1
 * @description undiscord ported to BD
 */
module.exports = class undiscordify {

    getName() {
        return "Undiscord";
    }
    getDescription() {
        return "undiscord lmfao i guess";
    }
    getVersion() {
        return "0.0.1";
    }
    ...
    start() {
        console.log('Undiscord is now enabled')
    }
    stop() {
        console.log('Undiscord is now disabled')
    }

    load() {
        console.log('Undiscord started')
        async function deleteMessages(authToken, authorId, guildId, channelId, minId, maxId, content, hasLink, hasFile, includeNsfw, includePinned, searchDelay, deleteDelay, extLogger, stopHndl, onProgress) {
            ...
            const escapeHTML = html => html.replace(/[&<"']/g, m => ({
                '&': '&amp;',
                '<': '&lt;',
                '"': '&quot;',
                '\'': '&#039;'
            ...
            const log = {
                debug() {
                    extLogger ? extLogger('debug', arguments) : console.debug.apply(console, arguments);
                },
                info() {
                    extLogger ? extLogger('info', arguments) : console.info.apply(console, arguments);
                },
                verb() {
                    extLogger ? extLogger('verb', arguments) : console.log.apply(console, arguments);
                },
                warn() {
                    extLogger ? extLogger('warn', arguments) : console.warn.apply(console, arguments);
                },
                error() {
                    extLogger ? extLogger('error', arguments) : console.error.apply(console, arguments);
                },
                success() {
                    extLogger ? extLogger('success', arguments) : console.info.apply(console, arguments);
                },
            };
    ...
    <div id="undiscord" style="display:none;">
        ...
        <div class="form">
            <div style="display:flex;flex-wrap:wrap;">
                ...
                <span>Delete Delay (recommended to use at 1700)<a
                href="https://github.com/victornpb/deleteDiscordMessages/blob/master/help/delay.md" title="Help"
                target="_blank">?</a><br>
                    <input id="deleteDelay" type="number" value="1700" step="100">
                </span>
            </div>
            <hr>
